...
func (r *leaderboardRepo) UpdateUserGameStats(ctx context.Context, leaderboard *entities.Leaderboard) error {
	// Check if the entry exists by directly querying the leaderboard table
	var scoreID string
	//checkQuery := `SELECT score_id FROM leaderboard WHERE score_id = $1`

	checkQuery := (&db.SelectQueryBuilder{
		Columns: "score_id",
		From:    "leaderboard",
		Where:   "score_id = $1",
	}).Build()
	err := r.db.QueryRowContext(ctx, checkQuery, leaderboard.ScoreID).Scan(&scoreID)
	...
	if scoreID != "" {
		...
		updateQuery := (&db.UpdateQueryBuilder{
			Table: "leaderboard",
			Set:   "wins = $1, losses = $2, score = $3",
			Where: "score_id = $4",
		}).Build()
		...
		insertQuery := (&db.InsertQueryBuilder{
			Table:   "leaderboard",
			Columns: "score_id, user_id, game_id, wins, losses, score",
		}).Build()
